...
export const POST: RequestHandler = (async ({ request, cookies }: RequestEvent): Promise<Response> => {
    const { setId, word, language, translations, explanation } = await request.json();
    if (cookies.get('token') !== '' && cookies.get('token') !== null && cookies.get('token') !== undefined) {
        ...
        if (user !== null) {
            return json(((await createWord(setId, word, user.email, language, translations, explanation))).toString());
        } else {
            return json('');
        }
    } else {
